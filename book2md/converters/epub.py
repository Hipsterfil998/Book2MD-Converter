"""epub.py — EPUB → Markdown via ebooklib + vLLM."""
...
class EpubToMarkdownConverter(PipelineStep):

    def __init__(
        self,
        model_id: str = epub_config.model_id,
        max_chunk_chars: int = epub_config.max_chunk_chars,
        max_new_tokens: int = epub_config.max_new_tokens,
    ...
    def run(self, epub_path: str, output_path: str | None = None) -> str:
        return self.convert(epub_path, output_path)
    ...
    def convert(self, epub_path: str, output_path: str | None = None) -> str:
        ...
        messages = [[{"role": "user", "content": epub_config.prompt.format(html=c)}] for c in chunks]

        sampling_params = SamplingParams(max_tokens=self.max_new_tokens, temperature=0.0,
                                         repetition_penalty=epub_config.repetition_penalty)
