"""extractor.py — Bibliographic metadata extractor using Qwen3 via vLLM.
...
from book2md.config import epub_config, metadata_config, eval_config
from book2md.base import PipelineStep
from vllm import LLM, SamplingParams
from book2md.utils import suppress_worker_stderr


class MetadataExtractor(PipelineStep):
    """Extracts bibliographic metadata from pre-saved eval pages."""

    def __init__(self, model_id: str = epub_config.model_id, max_new_tokens: int = metadata_config.max_new_tokens):
    ...
    def run(self, output_dir: str, output_csv: str) -> pd.DataFrame:
        ...
        biblio_dataset = [
            [
                {"role": "system", "content": metadata_config.biblio_prompt},
        ...
        genre_dataset = [
            [
                {"role": "system", "content": metadata_config.genre_prompt},
