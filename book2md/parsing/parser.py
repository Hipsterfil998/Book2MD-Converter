"""parser.py — Dependency parsing of Markdown files using Stanza.
...
from book2md.config import parse_config
from book2md.base import PipelineStep

DetectorFactory.seed = 0  # make language detection reproducible


class DependencyParser(PipelineStep):

    def __init__(self, langs: list = None, output_format: str = parse_config.output_format):
        """
        Args:
            langs: list of Stanza language codes, e.g. ["it", "de"]
            output_format: "conllu", "json", or "both"
        """
        self.langs = langs if langs is not None else parse_config.langs
