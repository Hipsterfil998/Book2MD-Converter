"""Tests for PDFToMarkdownConverter static methods (no GPU needed)."""
from book2md.converters.pdf import PDFToMarkdownConverter


class TestClean:
    def test_strips_markdown_fence(self):
        assert PDFToMarkdownConverter._clean("```markdown\n# Title\nContent\n```") == "# Title\nContent"

    def test_strips_plain_fence(self):
        assert PDFToMarkdownConverter._clean("```\n# Title\n```") == "# Title"
...
class TestResolveImageRefs:
    def test_replaces_placeholder(self):
        result = PDFToMarkdownConverter._resolve_image_refs("Text [IMAGE_1] end", ["fig1.png"])
        assert "![image_1](images/fig1.png)" in result
        assert "[IMAGE_1]" not in result

    def test_appends_when_placeholder_missing(self):
        result = PDFToMarkdownConverter._resolve_image_refs("No placeholder here", ["fig1.png"])
        assert "![image_1](images/fig1.png)" in result
    ...
    def test_multiple_images_replaced_in_order(self):
        result = PDFToMarkdownConverter._resolve_image_refs("[IMAGE_1] text [IMAGE_2]", ["a.png", "b.png"])
    ...
    def test_missing_placeholder_appended_at_end(self):
        result = PDFToMarkdownConverter._resolve_image_refs("[IMAGE_1] text", ["a.png", "b.png"])
        assert "![image_1](images/a.png)" in result
        assert "![image_2](images/b.png)" in result
